(ns rackhdui.components
  (:require [rackhdui.api :as api]
            [rackhdui.utils :as utils]
...
(defonce form-data (atom {:host-state      {:form-state []}
                          :action-state    false
                          :row-selected    nil
...
(defc main-body < rum/reactive [app-state form-data]
  (let [hosts (rum/cursor-in app-state [:nodes])
        page-state (rum/cursor-in app-state [:page-state])]
